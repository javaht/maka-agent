...
| 04 | `04-safestorage-encryption.md` | Electron safeStorage primitive (Keychain/DPAPI/libsecret) + 4 distinct call sites (Copilot tokens / Claude subscription / MCP OAuth base64-wrapped) + symmetric isEncryptionAvailable check + silent plaintext fallback (no marker — flagged as risk) + **provider.apiKey IS plaintext in DB** (contradicts api-spec "encrypted, do not expose" claim) + legacy token migration pattern | **shipped** |
