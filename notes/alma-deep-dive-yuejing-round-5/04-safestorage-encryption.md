# 04 — Alma API key encryption: Electron safeStorage + plaintext fallback
...
But subtle: this means if a user MIGRATES their alma data (e.g.,
...
This is a deliberate UX tradeoff: alma works on Linux without
...
The DB file `~/.config/alma/alma.db` IS protected by
...
So old tokens (from a previous alma version with different
...
- The `provider.apiKey` column is plaintext, same as alma.
...
- The `requirePassword` and `sessionTimeout` settings hint at
  ...
  Round-5 candidate.
- Linux `libsecret` fallback: when it's missing, does alma show
...
- Round 2: [`05-bash-tool-family.md`](../alma-deep-dive-yuejing-round-2/05-bash-tool-family.md)
  — Bash tool can read `~/.config/alma/alma.db` directly,
